...
kernel_env new_kernel_env(uint32_t block_size, uint32_t elem_pthread,
    ...
    env->bits = bits;
    env->num_blocks = (number_keys + block_size * elem_pthread - 1) /
    ...
    cudaSucceeded(cudaMemcpy(env->d_keys, env->h_keys,
                             env->h_keys_size * sizeof(uint32_t),
    ...
    cudaSucceeded(cudaMalloc((void **)&env->d_output,
                             env->d_keys_size * sizeof(uint32_t)));
    cudaSucceeded(cudaMalloc((void **)&env->d_hist_transpose,
                             env->d_hist_size * sizeof(uint32_t)));
    cudaSucceeded(cudaMalloc((void **)&env->d_scan_res,
...
kernel_env copy(kernel_env env) {
    ...
    // - Allocate and init on device (GPU)
    cudaSucceeded(cudaMalloc((void **)&env_copy->d_keys,
                             env->d_keys_size * sizeof(uint32_t)));
    cudaSucceeded(cudaMemcpy(env_copy->d_keys, env->d_keys,
                             env->d_keys_size * sizeof(uint32_t),
                             cudaMemcpyDeviceToDevice));
    cudaSucceeded(cudaMalloc((void **)&env_copy->d_hist,
                             env->d_hist_size * sizeof(uint32_t)));
    cudaSucceeded(cudaMemcpy(env_copy->d_hist, env->d_hist,
                             env->d_hist_size * sizeof(uint32_t),
                             cudaMemcpyDeviceToDevice));
    cudaDeviceSynchronize();
...
uint32_t number_classes(kernel_env env) { return env->number_classes; }

uint32_t d_hist_size(kernel_env env) { return env->d_hist_size; }

uint32_t d_keys_size(kernel_env env) { return env->d_keys_size; }

uint32_t *d_keys(kernel_env env) { return env->d_keys; }

uint32_t *d_hist(kernel_env env) { return env->d_hist; }

uint32_t bits(kernel_env env) { return env->bits; }

uint32_t elem_pthread(kernel_env env) { return env->elem_pthread; }

void d_vec(uint32_t *source, uint32_t *dest, uint32_t size) {
    cudaMemcpy(dest, source, size * sizeof(uint32_t), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
}
...
void log_d_hist(kernel_env env) {
    ...
    cudaMemcpy(res, env->d_hist, env->d_hist_size * sizeof(uint32_t),
...
void log_d_hist_scan(kernel_env env) {
    uint32_t res[env->d_hist_size];
    cudaMemcpy(res, env->d_hist_scan, env->d_hist_size * sizeof(uint32_t),
               cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    log_vector_with_break(res, env->d_hist_size, env->number_classes);
}
...
void log_d_hist_transpose(kernel_env env) {
    uint32_t res[env->d_hist_size];
    cudaMemcpy(res, env->d_hist_transpose, env->d_hist_size * sizeof(uint32_t),
               cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    log_vector_with_break(res, env->d_hist_size, env->number_classes);
}
